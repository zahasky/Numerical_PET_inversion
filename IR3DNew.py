...
from DataLoaderNew import InputDataset
...
parser.add_argument('--idx-train', type=int, default=24000, help='Index of the last training data')
parser.add_argument('--idx-val', type=int, default=29600, help='Index of the last validation data')
parser.add_argument('--train-batch-size', type=int, default=32, help='Number of the batches for training')
parser.add_argument('--val-batch-size', type=int, default=32, help='Number of the batches for validation')
parser.add_argument('--num-epochs', type=int, default=294, help='Number of epochs for both the training and validation')
# Optimizer
...
parser.add_argument("--patience", type=float, default=15, help='Number of epochs before the learning rate is reduced due to no improvement')
# Loss function
...
# Save all the potential satisfactory results
parser.add_argument('--loss-save', type=float, default=2, help='Trigger for saving based on the training loss')
parser.add_argument('--loss-savev', type=float, default=1, help='Trigger for saving based on the validation loss')
...
train_dataloader = DataLoader(dataset=train_dataset, batch_size=args.train_batch_size, shuffle=True, num_workers=2, drop_last=True)
validation_dataloader = DataLoader(dataset=validation_dataset, batch_size=args.val_batch_size, shuffle=True, num_workers=2, drop_last=True)
...
#----------------------------------------
# Diectories for Training Results Storage
#----------------------------------------
...
for epoch in range(1,args.num_epochs+1):
    ...
    for i, (imgs_v) in enumerate(validation_dataloader):
        ...
        with torch.no_grad():
            ...
            r2 += r2_accu(fake_targetv[true_targetv.nonzero(as_tuple=True)], true_targetv[true_targetv.nonzero(as_tuple=True)])
    ...
    # Save the satisfactory results
    if g_lossv.item() < args.loss_savev:
        enc_dir = result_dir + '/encoder_epoch_' + str(args.num_epochs) + '_' + str(epoch) + '.pth'
        dec_dir = result_dir + '/decoder_epoch_' + str(args.num_epochs) + '_' + str(epoch) + '.pth' 

        torch.save(encoder.state_dict(), enc_dir)
        torch.save(decoder.state_dict(), dec_dir)

# Print out the loss and accuracy for plotting
print(g_l)
print(g_lv)
print(ssim_v)
print(r2_v)
