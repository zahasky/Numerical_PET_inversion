"""
This code is a 3D Kalman filter model.
The ensemble dimension is 20x20x40 
...
# # Terra Machine Path
# # names of executable with path IF NOT IN CURRENT DIRECTORY
exe_name_mf = 'C:\\Users\\zhuang296\\Documents\\NSKF_Edw\\mf2005'
exe_name_mt = 'C:\\Users\\zhuang296\\Documents\\NSKF_Edw\\mt3dms'
...
# directory to save data
directory_name = '/Users/zhuang296/Desktop/Kalman_testing_Edwards'
initial_ens_dir = '/Users/zhuang296/Desktop/Initial_Edwards_Ens'
final_ens_dir = '/Users/zhuang296/Desktop/Final_Edwards_Ens'

workdir = os.path.join('.', directory_name)
inidir = os.path.join('.', initial_ens_dir)
findir = os.path.join('.', final_ens_dir)

# if the path exists then we will move on, if not then create a folder with the 'directory_name'
if os.path.isdir(workdir) or os.path.isdir(inidir) or os.path.isdir(findir) is False:
    os.mkdir(workdir)
    os.mkdir(inidir)
    os.mkdir(findir)

    print("Directory '% s' created" % workdir)
    print("Directory '% s' created" % inidir)
    print("Directory '% s' created" % findir)
...
arrival_data = np.loadtxt(new_path + '/Edwards_2ml_2_3mm_at_norm.csv', delimiter=',')
...
hk_cnn = np.loadtxt('./results/dec_hk_Edw.csv', delimiter=',')
...
Ps = lhs.generate([(-6., -1.), (1.0, 25.0), (1.0, 25.0), (1.0, 25.0), (0.0, 1.57), (0.0, 1.57), (0.0, 1.57)], Ne + 10)

# example of how to efficiently save data to text file
np.savetxt(workdir + '/Edwards_ensemble_3D_parameter_space', Ps , delimiter=',', fmt='%.3e')
...
kmD_exp_mean_obs = np.log10(hk_obs/(9.869233E-13*9.81*100*60/8.9E-4))
...
for i in range(Ne):
    ...
    # field = 10**(srf.structured([np.arange(dnlay), np.arange(dnrow), np.arange(dncol)]) + kmD_exp_mean_obs)
    field = 10 ** (srf.structured([np.arange(dnlay), np.arange(dnrow), np.arange(dncol)]))
    sample = kmD_cnn * np.array(field)
    # sample = np.array(field)
    ...
    # Save all initial ensembles
    np.savetxt(inidir + '/Ens' + str(i) + '.csv', Z[:-1, i], delimiter=',')
...
def Kalman_generator(Z, Yj, Y, R, invR, hk_update, iteration, num_iterations):
    ...
    for i in range(Ne):
        ...
        while Lif > Li[i] and count < 5:
            ...
            if iteration == num_iterations-1:
                # Save all initial ensembles
                np.savetxt(findir + '/Ens' + str(i) + '.csv', Z[:-1, i], delimiter=',')
...
for iteration in range(0, num_iterations):
    ...
    Z, Yj, Li = Kalman_generator(Z, Yj, Y, R, invR, hk_update, iteration, num_iterations)
