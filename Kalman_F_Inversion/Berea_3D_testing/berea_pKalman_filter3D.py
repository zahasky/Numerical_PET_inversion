...
at_obs = arrival_data.reshape(nlay, nrow, ncol)
at_obs = np.flip(at_obs, 0)

# Import data
norm_ct = np.loadtxt(new_path + '\\berea_dry_norm_coarse.csv', delimiter=',')
norm_ct = norm_ct.reshape(nlay, nrow, ncol)

pfun.plot_2d(norm_ct[:,10,:], grid_size[0], grid_size[1], 'norm CT', cmap='viridis')
...
pfun.plot_2d(at_obs[10,:,:], grid_size[0], grid_size[2], 'arrival time', cmap='bwr')
plt.clim(-0.2, 0.2)
pfun.plot_2d(at_obs[:,10,:], grid_size[0], grid_size[2], 'arrival time', cmap='bwr')
plt.clim(-0.2, 0.2)

# print(at_obs.shape)
at_nonzero_ind = np.nonzero(at_obs)
at_obs_wo_zeros = at_obs[at_nonzero_ind]

# print(at_obs_wo_zeros.shape)
# =============================================================================
...
num_iterations = 10
# observed data
Y = np.append(at_obs_wo_zeros, hk_mean_obs)

num_obs = Y.size
num_parameters = len(at_obs_wo_zeros)
...
Ps_exp = lhs.generate([(-3., -2.0), (2.0, 5.0), (50.0, 150.0), (50.0, 150.0)], Ne+10)
Ps_exp = np.array(Ps_exp)

# range a = 1-4.5, b = 15-25 in form np.exp(norm_ct.flatten()*a)+b
Ps1 = lhs.generate([(1.0, 4.5), (15.0, 25.0)], int(pNe/2))
# range a = 5 - 45, = 23-30 in form norm_ct.flatten()*a)+ (b-a/2)
Ps2 = lhs.generate([(5, 45), (23, 30.0)], int(pNe/2))

Ps_concat = np.concatenate((Ps1, Ps2), axis=0)
ind_col = np.zeros((pNe,1))
ind_col[int(pNe/2):] = 1
Psc = np.append(ind_col, Ps_concat, axis=1)
Psc = np.append(Psc, np.zeros((pNe,1)), axis=1)

# combine field property structures
Ps = np.concatenate((Ps_exp, Psc), axis=0)


# example of how to efficiently save data to text file    
np.savetxt('Berea_ensemble_3D_ct_normscale100_700', Ps , delimiter=',', fmt='%.3e')


kmD_exp_mean_obs = np.log10(hk_mean_obs/(9.869233E-13*9.81*100*60/8.9E-4))

for i in range(Ne):
    # draw out single  field properties
    p = Ps[i]
    
    if p[0] == 0:
        # exponential scale
        field = np.exp(norm_ct* p[1]) + p[2]
    elif p[0] == 1:
        # linear scale
        field = (norm_ct* p[1]) + (p[2] - p[1]/2)
    else:
    ...
    zf = np.array(hk_initial[at_nonzero_ind])
    zfl = np.log(zf)
    zfl[zf==0] =0
    Z[:,i] = np.log(zf)
    # Z[:,i] = zf
    ...
    yf = np.append(at_ensem[at_nonzero_ind], hk_mean)
    Yj[:,i] = yf
...
def Kalman_generator(Z, Yj, Y, R, invR):
    ...
    for i in range(Ne):
        ...
        while Lif > Li[i] and count < 3:
            ...
            zfl = np.exp(Z[:,i])
            zfl[zf==0] =0
            hk_update =  hk_initial
            hk_update[at_nonzero_ind] = zfl
...
# reshape mean hk with zeros in the right places
meanZ = np.mean(np.exp(Z), axis=1)
hk_initial_mean =  hk_initial
hk_initial_mean[at_nonzero_ind] = meanZ
...
pfun.plot_2d(hk_initial_mean[:,10,:], grid_size[0], grid_size[2], '[cm/min]', cmap='gray')    
...
meanZ = np.mean(np.exp(Z), axis=1)
hk_mean =  hk_initial
hk_mean[at_nonzero_ind] = meanZ
# hk_mean = np.reshape(np.mean(np.exp(Z), axis=1), (nlay, nrow, ncol))
pfun.plot_2d(hk_mean[:,10,:], grid_size[0], grid_size[2], '[cm/min]', cmap='gray')    
plt.title('Iteration #' + str(iteration) +' mean K')
# plt.clim(het_hk[:,1,:].min(), het_hk[:,1,:].max())
# Ensemble
stdZ = np.std(np.exp(Z), axis=1)
hk_std =  hk_initial
hk_std[at_nonzero_ind] = stdZ
# hk_std = np.reshape(np.std(np.exp(Z), axis=1), (nlay, nrow, ncol))
pfun.plot_2d(hk_std[:,10,:], grid_size[0], grid_size[2], '[cm/min]', cmap='gray')    
plt.title('Iteration #' + str(iteration) +' STD K')

# Random hk realization
# pfun.plot_2d(het_hk[:,1,:], grid_size[0], grid_size[2], '[cm/min]', cmap='gray')    
# plt.title('True K')
# plt.clim(het_hk[:,1,:].min(), het_hk[:,1,:].max())

# rerun model with new parameters
pfun.plot_2d(hk_mean[:,:,20], grid_size[0], grid_size[1], 'mean', cmap='gray')
mf, mt, conc, btc, times, hk_test = model.mt3d_conservative('tracer', hk_mean, prsity, al, 
                        grid_size, v, perlen_mt, timprs, mixelm, exe_name_mf, exe_name_mt, workdir)

at_ensem = model.flopy_arrival_map_function(conc, times, grid_size, 0.5)

pfun.plot_2d(at_ensem[10,:,:], grid_size[0], grid_size[2], 'arrival time', cmap='bwr')
plt.clim(-0.2, 0.2)
pfun.plot_2d(at_ensem[:,10,:], grid_size[0], grid_size[2], 'arrival time', cmap='bwr')
plt.clim(-0.2, 0.2)
pfun.plot_2d(at_ensem[:,:,20], grid_size[0], grid_size[1], 'arrival time', cmap='bwr')

plt.plot(at_obs[at_nonzero_ind], at_ensem[at_nonzero_ind], '.k')
plt.plot([-0.5, 0.5], [-0.5, 0.5], 'r')

Rfrac = np.sum((at_obs[at_nonzero_ind] - at_ensem[at_nonzero_ind])**2)/ np.sum((at_obs[at_nonzero_ind] - np.mean(at_obs[at_nonzero_ind]))**2)
1-Rfrac
...
# =============================================================================
# save some data 
save_filename_btdn = 'allZ_normscale100_700.csv'
# save_data = np.append(hk_mean.flatten('C'), [[nrow, ncol, nlay], grid_size])
save_data = np.append(hk_mean.flatten('C'), [nrow, ncol, nlay, Ne])
