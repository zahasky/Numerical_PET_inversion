...
for w in range(dnrow):
    print('Inverting for the slice ' + str(w + 1))
    # 2D case
    at_obs_slice = at_obs[:, w, :]
    ...
    ib = np.reshape(ibound[:, w, :], (nlay, nrow, ncol))
    ...
    np.savetxt(workdir + '/hk_initial' + '_slice' + str(w + 1) + '.csv', Z_m, delimiter=',')
        ...
        try:
            ...
            for i in range(Ne):
                ...
                while Lif > Li[i] and count < 5:
                    ...
                    mf, mt, conc, times, hk_mean_obs = model.mt3d_conservative('tracer', hk_update, prsity, grid_size,
                                                                               al,
                                                                               v, ndummy_in, perlen_mt, timprs, mixelm,
                                                                               exe_name_mf, exe_name_mt, model_ws)
            ...
            np.savetxt(workdir + '/hk_problem' + str(error_count) + '_slice' + str(w + 1) + '.csv', Z[:, i],
                       delimiter=',')
    ...
    for iteration in range(0, num_iterations):
        ...
        np.savetxt(workdir + '/hk_inv' + str(iteration + 1) + '_slice' + str(w + 1) + '.csv', np.mean(Z, axis=1),
                   delimiter=',')
    ...
    hk_idx_prev = hk_idx
    hk_idx += 800
    hk_inverted[hk_idx_prev:hk_idx] = np.mean(Z, axis=1)

# Save the final inverted field
np.savetxt(workdir + '/hk_inv_3D.csv', hk_inverted, delimiter=',')
