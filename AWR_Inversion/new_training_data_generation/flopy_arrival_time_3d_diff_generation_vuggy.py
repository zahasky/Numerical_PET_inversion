...
workdir = os.path.join('D:\\Training_data_generation_3D\\tdata_vuggy_5k', directory_name)
# directory for homogeneous models
homogeneous_arrival_dir = os.path.join('D:\\Training_data_generation_3D\\tdata_vuggy_5k\\homogeneous_porosity_at')
...
new_perm_field_dir = os.path.join('D:\\Training_data_generation_3D\\tdata_vuggy_5k\\syn_core_perm_maps')
...
parameter_space_csv_name = 'parameter_space_5k_vuggy.csv'
...
while td < ntraining_data:
    ...
    if generate_fields == True:
        ...
        ##### ADD VUGGS
        # generate distribution and location based on number of vugs
        vugs = np.random.rand(int(p[10]),4)
        vugs[:,0] = vugs[:,0]*nlay # layer location
        vugs[:,1] = vugs[:,1]*nrow # row location
        vugs[:,2] = vugs[:,2]*ncol # column location
        # forth column is 0-1 for disrtibution sampling
        vugs[:,3] = sample_lognormal_cdf(vugs[:,3], 1, 10**p[11])
        vug_perm = (10**p[1])* (10**p[9])
        
        if vug_perm > 20000:
            vug_perm = 20000
            
        field = vug_map_generation_function(field, vugs, vug_perm)
        ...
        np.savetxt(new_perm_field_dir + '\\corevug_k_3d_m2_' + str(td)+'.csv', save_data , delimiter=',', fmt='%.3e')
    else:
        # Import permeability map
        field_km2 = np.loadtxt(new_perm_field_dir + '\\corevug_k_3d_m2_' + str(td) +'.csv', delimiter=',')
