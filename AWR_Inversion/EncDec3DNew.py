...
parser.add_argument("--num-blocks", type=int, default=4, help='Number of dense blocks in a residual dense block')
...
parser.add_argument('--lat-channels', type=int, default=96, help='Number of channels for the latent space')
...
class DenseBlock(nn.Module):
    def __init__(self, in_features, growth_rate, dropout_rate, kernel_size, stride, padding, non_linearity=True):
...
class ResiduleDenseBlock(nn.Module):
    def __init__(self, in_features, num_blocks, growth_rate, res_scale):
        ...
        DBlocks = [DenseBlock(in_features, growth_rate, args.dropout_rate, kernel_size, stride, padding)]

        for i in range(num_blocks-1):
            in_features += growth_rate

            if i == num_blocks-2:
                DBlocks.append(DenseBlock(in_features, growth_rate, args.dropout_rate, kernel_size, stride, padding, non_linearity=False))
                break

            DBlocks.append(DenseBlock(in_features, growth_rate, args.dropout_rate, kernel_size, stride, padding))
...
class Encoder(nn.Module):
    def __init__(self, inchannels=args.in_channels, outchannels=args.lat_channels, filters=args.num_filters, num_res_blocks=args.num_res_blocks):
        ...
        self.trans1 = nn.Sequential(
            nn.Conv3d(inchannels, filters, kernel_size=3, stride=2, padding=[1,1,0], bias=False)
        )
        ...
        self.trans2 = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv3d(filters, outchannels, kernel_size=2, stride=2, padding=0, bias=False)
        )
        # output size, outchannels x 5 x 5 x 10

    def forward(self, inp):
        # inp: inchannels x 20 x 20 x 40
        out1 = self.trans1(inp)        # filterd x 10 x 10 x 20
        out2 = self.res_blocks1(out1)  # filterd x 10 x 10 x 20
        out3 = self.trans2(out2)       # outchannels x 5 x 5 x 10
...
class Decoder(nn.Module):
    def __init__(self, inchannels=args.lat_channels, outchannels=args.out_channels, filters=args.num_filters, num_res_blocks=args.num_res_blocks):
        ...
        self.conv1 = nn.Conv3d(inchannels, filters, kernel_size=1, stride=1, padding=0, bias=False)
